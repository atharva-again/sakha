...
MAX_COMPLETION_LENGTH = 256          # Max tokens per completion (Qwen3-0.6B needs ~100-200 thinking tokens then action)
...
def run_llm_eval_batched(task, model, tokenizer, seeds, max_steps):
    ...
    for step in range(max_steps):
        ...
        # 256 is what we know works empirically (0.18 baseline). Qwen3-0.6B uses
        # ~100-200 tokens for thinking then emits the action call. Lower budgets
